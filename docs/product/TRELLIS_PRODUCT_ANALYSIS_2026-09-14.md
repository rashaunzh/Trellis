...
更新：2026-09-15。设计版本：学习辅助闭环版。核查基线：e24b110（之前多次提交仅改文档）。本文与[桌面交互规格](TRELLIS_DESKTOP_UPDATE_DESIGN_2026-09-14.md)共同替代上一轮以活动整理和 CK-001–200 为中心的设计。旧方案从当前文件退出，由 Git 历史追溯；文件名保留首次建立日期，避免跨设备入口失效。
...
支持等级属于每个活动，不属于付费身份或用户等级。路线审阅就展示“这个活动提供哪些辅助”；材料不足只能降低具体帮助范围，不能让用户进入后才发现所有功能都是空的。
...
每阶段展示预期结果、来源选择、首批活动、辅助服务等级、前置与缺口。用户在采用前就知道“哪些活动有完整辅助，哪些仅支持定位与记录”。
...
本轮不是竞品排名。选择设计模式时看它是否帮助学习链路；不用“别人有一个按钮”证明这里也应该有。

| 参照模式 | 可借鉴 | Trellis 必须额外解决 |
|---|---|---|
| 任务详情集中上下文 | 单一活动身份、清楚的回到列表方式 | 任务详情不足以承担尝试、反馈与修订，需要学习空间 |
| 材料收集与处理分离 | 先保存后处理、使用关系清楚 | 分析材料与能提供辅导之间还有质量门 |
| 外部课程教学 | 有既定教学内容与顺序 | 与个人目标的关联、应用迁移、学习中断与后续安排 |
| AI 对话解释 | 追问与按问题调整表达 | 作答归属、引用、标准、反馈落实、版本与正式状态 |

已有官方参照：[Todoist 任务详情](https://www.todoist.com/help/todoist/features/use-the-task-view-to-manage-tasks-in-todoist-eDeRDO0C)、[Readwise 材料组织](https://docs.readwise.io/reader/guides/workflows/library-configuration)、[Microsoft HAX](https://www.microsoft.com/en-us/haxtoolkit/ai-guidelines/)。这些支持信息组织和 AI 交互检查，不证明学习成效。真正学习机制的来源与适用边界见第 4 节。
...
| P1 从目标到合适起点 | 无/少材料用户能审阅支持程度明确的路线并进入 P0 过程 | 起点假设、可选尝试、来源范围、L0/L1/L2 标记、正式采用 | 全面自动诊断 |
...
本轮源码核查与官方资料阅读已完成。没有运行新浏览器流程、付费模型、真实学习研究、迁移或部署。文档预检只证明入口和引用可用，不证明设计有效。既有[截图证据](evidence/pm-independent-2026-09-14/README.md)仅保留其原采集范围。
