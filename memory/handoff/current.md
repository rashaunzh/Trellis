...
- `npm run test:domain`：59/59（+8：掌握确认 4 + 复测 4，含普通活动自动验证回归）。
- eslint 0；build 通过（路由含 nodes/:id/confirm-mastery、nodes/:id/retest）；.mjs 6/6。
- 浏览器 P0：综合任务→待确认(紫)→确认掌握→validated + mastery_confirm 记录；复测提醒卡→生成复测活动。
...
- [x] P0 三件套全部落地（2026-08-20）：综合情境任务（MVP 轮 integrated_task）+ 掌握确认（pending_confirmation 四色 + confirm-mastery API + 纠正降级补强）+ 延迟复测（retest 活动 + dueReviews 提醒 + 通过顺延/失败降级）。
...
- P0 三件套已全部实现（综合任务/掌握确认/延迟复测）。下一价值方向：内容包深化（AI-For-Beginners 全量映射）、AI 评估接入（工作台 API 配置已就绪）、自定义域名（演示对象在国内时）。
...
1. 提交推送 P0 轮（state-machine/types/schema 0009/d1/service/composer/2 API/前端四色+复测卡/测试/memory），显式 `git add`，push 走 Clash 代理，同步双克隆。
2. 重新部署线上：远程 D1 应用 0009 迁移 → `DATABASE_ID=<uuid> vinext build` → `wrangler deploy` → 线上验证（Bot Fight Mode 带 UA）。
3. 用户验收：本地 `http://localhost:3411/learn`（截图 `.wrangler/acceptance-shots-p0/`）或公网 URL；验收掌握确认与复测提醒。
