...
export const lineMeta: Record<Line, { name:string; short:string; purpose:string; color:string }> = {
  ...
  J: { name:"求职发展", short:"求职", purpose:"用真实 JD 倒推能力、作品证据与求职动作", color:"#805b1d" },
  B: { name:"商业探索", short:"商业", purpose:"通过低成本发布与真实反馈验证方向", color:"#81508f" },
  I: { name:"创新与想法", short:"想法", purpose:"连接外部 Inbox，判断想法进入哪条路线", color:"#345f96" },
};
...
export const routeStages: Record<Line, Array<{ id:string; order:number; name:string; topics:string }>> = {
  ...
  J: [
    { id:"J1", order:1, name:"目标岗位与 JD 样本", topics:"广州真实岗位 · 岗位分型 · 高频要求" },
  ...
  B: [
    { id:"B1", order:1, name:"X + 小红书自媒体", topics:"选题 · 最小发布 · 跨平台改写 · 反馈" },
    { id:"B2", order:2, name:"短视频 / 抖音", topics:"脚本 · 分镜 · 剪辑 · 数据复盘" },
    { id:"B3", order:3, name:"能源平台产品", topics:"研究 · 需求 · 原型 · 验证" },
  ],
  I: [
...
const roadmapRecords:TrellisRecord[] = [
  ...
  roadmapTask("J-02","建立广州 AI 产品经理能力地图","J","J2 · 能力地图","AI 产品经理求职","把真实 JD 归并为产品、AI、数据、工程协作与业务能力。","归一 JD 关键词 → 聚类能力 → 记录频率 → 为每项链接现有证据"),
  roadmapTask("J-03","确定三个最优先的能力缺口","J","J3 · 差距与优先级","AI 产品经理求职","区分必须补齐、需要作品证明和仅属加分的要求。","对照能力地图 → 评估当前证据 → 选三个缺口 → 连接学习或作品任务"),
  roadmapTask("J-05","把 Notebook 测评写成面试故事","J","J5 · 简历与面试叙事","AI 产品经理求职","将真实过程转换为问题、判断、行动、指标和结果，而不是罗列功能。","写背景与约束 → 说明关键判断 → 链接证据 → 写结果与反思 → 录一次口述"),
  roadmapTask("J-06","建立投递与反馈闭环","J","J6 · 投递与反馈闭环","AI 产品经理求职","用真实市场反馈修正路线、作品和目标岗位。","记录岗位与版本 → 保存面试反馈 → 区分偶发与重复信号 → 更新一项路线决策"),
  roadmapTask("B-02","把一份真实成果改成首条短视频","B","B2 · 短视频 / 抖音","内容渠道实验","验证脚本和表达，不先建设完整视频流水线。","从现有成果选一个结论 → 写 30 秒脚本 → 做最小剪辑 → 发布或保存待发 → 记录反馈"),
  roadmapTask("B-03","写清能源平台的首个产品假设","B","B3 · 能源平台产品","能源平台","把能源平台当作待验证产品，而不是默认长期运营主线。","选择目标用户 → 写核心问题 → 定义最小方案 → 列验证证据 → 决定是否建立项目"),
...
export const starterRecords: TrellisRecord[] = [
  ...
  {
    id:"NB-06", title:"发布 Notebook 测评作品与复盘", recordType:"artifact", line:"B", module:"B1 · X + 小红书 · 6/6",
    projectId:"Notebook 测评", scheduleRole:"maintain", status:"later", estimatedMinutes:60, actualMinutes:0,
    coreAction:"将真实测评过程转成作品页面，并提炼一份适合 X / 小红书的短内容。",
  ...
  {
    id:"J-01", title:"收集 8 份广州 AI 产品经理真实 JD", recordType:"task", line:"J", module:"J1 · 目标岗位与 JD 样本",
    projectId:"AI 产品经理求职", scheduleRole:"support", status:"near", estimatedMinutes:60, actualMinutes:0,
    coreAction:"先用少量真实岗位校准方向，不把高级岗位的要求误当作入门基线。",
    learningScope:"广州；AI 产品经理、AI 解决方案产品经理及相邻岗位。",
    executionMethod:"保存岗位链接和原文 → 标注行业、年限、职责与技能 → 去掉重复招聘 → 标出入门可达与明显高级岗位",
    completionCriteria:"8 份有效 JD 及一页岗位分型。",
    evidence:"", blockers:"Boss 链接失效时保存必要摘录和访问日期。", nextStep:"提取高频能力并建立证据映射。",
    aiReview:"", acceptance:"unreviewed", sourceUrl:"https://www.zhipin.com/web/geek/job", notes:""
  },
  {
  ...
  {
    id:"B-01", title:"确定 X + 小红书首个最小发布实验", recordType:"task", line:"B", module:"B1 · X + 小红书自媒体",
    ...
    executionMethod:"选择一个已有过程 → 定义目标读者 → 写 X 版 → 改写小红书版 → 发布或保存待发 → 记录反馈",
