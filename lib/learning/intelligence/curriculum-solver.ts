...
export function deriveTargetNodeIds(goal: string, graph: DomainGraph, proposed: string[] = []): string[] {
  ...
  for (const alias of goalAliases) {
    if (alias.pattern.test(goal.toLowerCase())) alias.nodes.filter((id) => valid.has(id)).forEach((id) => selected.add(id));
  }
  if (selected.size === 0) {
...
function courseScore(input: {
  ...
  const { course, mapping, supplied, usedUnits, selectedCourses } = input;
  ...
  return relationScore + levelScore + sourceScore + mapping.confidence * 4
    ...
    - Math.max(0, course.genome.prerequisites.length - 1);
}
...
export function solveCurriculum(input: {
  ...
  const targetNodeIds = prerequisiteClosure(input.targetNodeIds, input.graph);
  ...
  for (const nodeId of targetNodeIds) {
    let candidates = input.courses
      ...
      .map((mapping) => ({
        course, mapping,
        score: courseScore({ course, mapping, supplied, usedUnits, selectedCourses: new Set(selectedByCourse.keys()) }),
      })))
      .sort((a, b) => b.score - a.score || a.course.genome.id.localeCompare(b.course.genome.id));
    if (selectedByCourse.size >= 6) {
      candidates = candidates.filter((candidate) => selectedByCourse.has(candidate.course.genome.id));
    }
  ...
  const decisions: CurriculumAssembly["decisions"] = input.courses.map((course, index) => {
    ...
    if (unitIds.length === 0) {
      return {
        ...
        role: supplied.has(course.genome.id) ? "defer" as const : "exclude" as const,
        selectedUnitIds: [],
