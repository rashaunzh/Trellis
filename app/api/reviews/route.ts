...
export async function POST(request: Request) {
  try {
    const payload = await request.json() as { weekKey?:string; progress?:string; deviation?:string; adjustments?:string };
    ...
    const values = { progress:payload.progress ?? "", deviation:payload.deviation ?? "", adjustments:payload.adjustments ?? "", updatedAt:new Date().toISOString() };
